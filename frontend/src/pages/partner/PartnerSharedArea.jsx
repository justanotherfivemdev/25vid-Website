...
const PartnerSharedArea = () => {
  ...
  useEffect(() => {
    axios.get(`${API}/auth/partner/me`)
      .then(res => {
        if (res.data?.account_type !== 'partner') { navigate('/partner-login', { replace: true }); return; }
        setUser(res.data);
        fetchData();
      })
  ...
  useEffect(() => { if (user) fetchData(); }, [typeFilter]); // eslint-disable-line react-hooks/exhaustive-deps
...
export default PartnerSharedArea;
