...
const IntelBoard = () => {
  ...
  return (
    <div className={inLayout ? '' : 'min-h-screen bg-[#050a0e] text-[#d0d8e0]'}>
      {/* Nav */}
      {!inLayout && (
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#050a0e]/95 backdrop-blur-xl border-b border-[rgba(0,255,136,0.15)]">
        <div className="container mx-auto px-4 md:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold tracking-[0.12em] text-[#e8c547]" style={{ fontFamily: "'Orbitron', sans-serif" }} data-testid="intel-board-title">
              INTELLIGENCE BOARD
            </h1>
          </div>
      ...
      <div className={`${inLayout ? 'pt-4' : 'pt-20'} pb-12 px-4 md:px-6`}>
        <div className="container mx-auto max-w-5xl space-y-6">
          {/* Header Banner */}
          <div className="relative corner-bracket border border-[rgba(0,255,136,0.15)] bg-[radial-gradient(circle_at_top,rgba(0,255,136,0.06),#050a0e_58%)] px-6 py-7 shadow-2xl" data-testid="intel-board-header">
            ...
            <h2 className="mt-3 text-3xl font-black uppercase tracking-[0.12em] text-[#e8c547]" style={{ fontFamily: "'Orbitron', sans-serif" }}>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              ...
              <Input value={search} onChange={e => setSearch(e.target.value)} className="bg-[#050a0e] border-[rgba(0,255,136,0.15)] pl-10 rounded-none text-[#d0d8e0]" style={{ fontFamily: "'JetBrains Mono', monospace" }} placeholder="Search briefings..." data-testid="intel-board-search" />
            </div>
            <div className="flex flex-wrap gap-2">
              <Button size="sm" variant={!filterCat ? 'default' : 'outline'} onClick={() => setFilterCat('')}
                className={!filterCat ? 'bg-[#111a24] text-[#00ff88] border border-[rgba(0,255,136,0.3)]' : 'border-[rgba(0,255,136,0.12)] text-[#8a9aa8]'} data-testid="filter-all">
                ALL
              </Button>
              {CATEGORIES.map(c => (
                <Button key={c.value} size="sm" variant={filterCat === c.value ? 'default' : 'outline'}
                  onClick={() => setFilterCat(filterCat === c.value ? '' : c.value)}
                  className={filterCat === c.value ? `${c.color}` : 'border-[rgba(0,255,136,0.12)] text-[#8a9aa8]'}
          ...
          {filterTag && (
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline" className="border-[rgba(0,255,136,0.4)] text-[#00ff88] text-xs">
            ...
            <div className="flex flex-wrap gap-1.5">
              {allTags.map(t => (
                <button key={t} onClick={() => setFilterTag(filterTag === t ? '' : t)}
                  className={`text-[10px] px-2 py-0.5 rounded border transition-colors ${filterTag === t ? 'bg-[rgba(0,255,136,0.08)] border-[rgba(0,255,136,0.4)] text-[#00ff88]' : 'border-[rgba(0,255,136,0.1)] text-[#4a6070] hover:border-[rgba(0,255,136,0.3)] hover:text-[#8a9aa8]'}`}
                  data-testid={`tag-${t}`}>
                  {t}
                </button>
              ))}
          ...
          {/* Briefings */}
          {loading ? (
            <div className="text-center py-12 text-[#00ff88]" style={{ fontFamily: "'JetBrains Mono', monospace" }}><span className="animate-pulse">■</span> Loading intelligence feed...</div>
          ) : briefings.length === 0 ? (
            <div className="text-center py-16 text-[#4a6070] border border-dashed border-[rgba(0,255,136,0.1)]">
              ...
              {briefings.map((b, idx) => {
                ...
                return (
                  <button key={b.id} onClick={() => setSelected(b)} className="w-full text-left group" data-testid={`briefing-${idx}`}>
                    <Card className={`bg-[#0c1117] border-[rgba(0,255,136,0.12)] hover:border-[rgba(0,255,136,0.35)] transition-all duration-300 overflow-hidden ${b.user_acknowledged ? 'border-l-2 border-l-green-600' : ''}`}>
                      ...
                      <CardContent className="p-5">
                        <div className="flex items-start gap-4">
                          <div className="flex-1 min-w-0">
                            ...
                            <h3 className="text-lg font-bold tracking-wide group-hover:text-[#e8c547] transition-colors" style={{ fontFamily: "'Orbitron', sans-serif" }}>{b.title}</h3>
                            ...
                            <div className="flex items-center gap-4 mt-3 text-xs text-[#4a6070]">
                              ...
                              {(b.tags || []).length > 0 && (
                                <span className="flex items-center gap-1 text-[rgba(0,255,136,0.5)]">
                                  <Tag className="w-3 h-3" />{b.tags.slice(0, 3).join(', ')}{b.tags.length > 3 && ` +${b.tags.length - 3}`}
                                </span>
                              )}
                            </div>
                          </div>
                          <ChevronRight className="w-5 h-5 text-[#4a6070] group-hover:text-[#00ff88] transition-colors shrink-0 mt-2" />
                        </div>
                      </CardContent>
                    </Card>
                  </button>
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="bg-[#0c1117] text-[#d0d8e0] border-[rgba(0,255,136,0.2)] max-w-3xl max-h-[85vh] overflow-y-auto rounded-none">
          {selected && (
            <>
              <DialogHeader>
                <div className="flex items-center gap-2 flex-wrap mb-2">
                ...
                <DialogTitle className="text-2xl tracking-wide" style={{ fontFamily: "'Orbitron', sans-serif" }} data-testid="intel-detail-title">{selected.title}</DialogTitle>
              ...
              <div className="mt-4 border-t border-[rgba(0,255,136,0.1)] pt-4">
                ...
                {selected.tags?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-6 pt-4 border-t border-[rgba(0,255,136,0.1)]">
                    {selected.tags.map(t => (
                      <Badge key={t} variant="outline" className="border-[rgba(0,255,136,0.3)] text-[#00ff88] text-[10px]">
                ...
                {/* Acknowledge Bar */}
                <div className="mt-6 pt-4 border-t border-[rgba(0,255,136,0.1)] flex items-center justify-between">
                  ...
                  <Button
                    ...
                    className={selected.user_acknowledged
                      ...
                      : 'bg-[#111a24] text-[#00ff88] border border-[rgba(0,255,136,0.3)] hover:bg-[rgba(0,255,136,0.08)]'}
