      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-900/95 backdrop-blur border-b border-red-900/30">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            {user?.role === 'admin' && <Link to="/admin"><Button size="sm" variant="outline" className="border-red-700 text-red-500 hover:bg-red-700/10"><Shield className="w-4 h-4" /></Button></Link>}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filtered.map(m => (
                <Link to={`/roster/${m.id}`} key={m.id}>
                  <Card className="bg-gray-900/80 border-gray-800 hover:border-red-700/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-red-900/10 group" data-testid={`roster-card-${m.id}`}>
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-bold text-base tracking-wide truncate group-hover:text-red-400 transition-colors" style={{ fontFamily: 'Rajdhani, sans-serif' }}>{m.username}</div>
                        ...
                        <ChevronRight className="w-4 h-4 text-gray-600 group-hover:text-red-500 transition-colors shrink-0 mt-1" />
                      </div>
                      <div className="flex items-center gap-2 mt-3">
                        ...
                        {m.role === 'admin' && <Badge className="bg-red-900/50 text-red-400 text-[10px] px-2 py-0">ADMIN</Badge>}
                      </div>
                    </CardContent>
                  </Card>
                </Link>
