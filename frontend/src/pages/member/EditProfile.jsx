      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#050a0e]/92 backdrop-blur-xl border-b border-tropic-gold/15">
        <div className="container mx-auto px-4 md:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold tracking-widest" style={{ fontFamily: "'Share Tech', sans-serif" }}>EDIT PROFILE</h1>
          </div>
        ...
        <div className="container mx-auto max-w-2xl space-y-6">
          ...
          <Card className="bg-[#0c1117]/80 border-[rgba(201,162,39,0.12)]">
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div><span className="text-[#4a6070] text-xs block">Username</span>{profile.username}</div>
                <div><span className="text-[#4a6070] text-xs block">Email</span>{profile.email}</div>
                <div><span className="text-[#4a6070] text-xs block">Rank</span><span className="text-[#8a9aa8]">{profile.rank || 'Not assigned'}</span></div>
                <div><span className="text-[#4a6070] text-xs block">Specialization</span><span className="text-[#8a9aa8]">{profile.specialization || 'Not assigned'}</span></div>
                <div><span className="text-[#4a6070] text-xs block">Status</span><span className="text-[#8a9aa8]">{profile.status || 'recruit'}</span></div>
                <div><span className="text-[#4a6070] text-xs block">Squad</span><span className="text-[#8a9aa8]">{profile.squad || 'Unassigned'}</span></div>
              </div>
              <p className="text-xs text-[#4a6070] mt-4">Rank, specialization, status, and squad are managed by unit command. Contact an admin to update these fields.</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#0c1117]/80 border-[rgba(201,162,39,0.12)]">
            ...
            <CardContent className="space-y-5">
              ...
              <div>
                <Label>Bio</Label>
                <p className="text-xs text-[#4a6070] mb-1">A short personal or in-character description.</p>
                <Textarea value={profile.bio || ''} onChange={e => setProfile({ ...profile, bio: e.target.value })} rows={3} className="bg-[#050a0e] border-[rgba(201,162,39,0.15)]" placeholder="Tell the unit about yourself..." data-testid="profile-bio-input" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label>Timezone</Label>
                  <p className="text-xs text-[#4a6070] mb-1">Helps with op scheduling.</p>
                  <Input value={profile.timezone || ''} onChange={e => setProfile({ ...profile, timezone: e.target.value })} className="bg-[#050a0e] border-[rgba(201,162,39,0.15)]" placeholder="e.g., EST, UTC+2, PST" data-testid="profile-tz-input" />
                </div>
                <div>
                  <Label>Preferred Role / Loadout</Label>
                  <p className="text-xs text-[#4a6070] mb-1">Your go-to kit or position.</p>
                  <Input value={profile.favorite_role || ''} onChange={e => setProfile({ ...profile, favorite_role: e.target.value })} className="bg-[#050a0e] border-[rgba(201,162,39,0.15)]" placeholder="e.g., Pointman, DMR, Medic" data-testid="profile-role-input" />
                </div>
              </div>
          ...
          {discordAvailable && (
          <Card className="bg-[#0c1117]/80 border-[rgba(201,162,39,0.12)]" data-testid="discord-section">
            ...
            <CardContent>
              {profile.discord_linked ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-4 bg-[#050a0e]/30 rounded-lg p-4 border border-[#5865F2]/30">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-[#4a6070]">ID: {profile.discord_id}</div>
                    </div>
                ...
                <div className="space-y-3">
                  <p className="text-sm text-[#8a9aa8]">Link your Discord account for quick login and unit identification.</p>
                  <Button
          ...
          {profile.email?.endsWith('@25thid.local') && (
            <Card className="bg-[#0c1117]/80 border-yellow-700/40" data-testid="set-password-section">
              <CardHeader className="pb-3">
                <CardTitle className="text-lg tracking-wider flex items-center gap-2">
                  <Lock className="w-5 h-5 text-tropic-gold" /> SET EMAIL & PASSWORD
                </CardTitle>
                <p className="text-xs text-[#4a6070] mt-1">Your account was created via Discord. Add an email and password to enable password-based login and allow Discord unlinking.</p>
              </CardHeader>
              <CardContent>
