...
function FieldShell({ label, help, children }) {
  return (
    <div className="space-y-2">
      <label className="text-xs font-medium uppercase tracking-wider text-[#4a6070]">{label}</label>
      {children}
      {help ? <p className="text-[11px] text-[#4a6070]">{help}</p> : null}
    </div>
  );
}
...
function ReadOnlyRow({ label, value, mono = false }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded border border-zinc-800/70 bg-[#050a0e]/40 px-3 py-2">
      <span className="text-xs text-[#4a6070]">{label}</span>
      <span className={`text-right text-sm text-[#8a9aa8] ${mono ? 'font-mono text-xs' : ''}`}>{value || 'N/A'}</span>
    </div>
  );
}
...
function ReferenceCard({ item }) {
  return (
    <Card className="border-zinc-800 bg-[#050a0e]/40">
      <CardContent className="space-y-2 p-4">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm font-semibold text-white">{item.label}</span>
          <Badge variant="outline" className="border-zinc-700 text-[10px] text-[#8a9aa8]">{item.section}</Badge>
          <Badge variant="outline" className="border-zinc-700 text-[10px] text-[#4a6070]">{item.type}</Badge>
        </div>
        <div className="font-mono text-xs text-tropic-gold-dark">{item.path}</div>
        <p className="text-sm text-[#8a9aa8]">{item.description}</p>
        <div className="text-xs text-[#4a6070]">Default: {item.defaultValue}</div>
        {item.notes ? <p className="text-xs text-[#4a6070]">{item.notes}</p> : null}
      </CardContent>
    </Card>
  );
}
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" variant="outline" onClick={() => setHistoryOpen(true)} className="h-7 border-zinc-800 text-xs text-[#8a9aa8]">
            <History className="mr-1 h-3 w-3" /> History ({configHistory.length})
          </Button>
          <Button size="sm" variant="outline" onClick={() => setDiffOpen(true)} className="h-7 border-zinc-800 text-xs text-[#8a9aa8]">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="grid w-full grid-cols-3 bg-[#050a0e]/60">
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <Card className="border-zinc-800 bg-[#050a0e]/60">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-semibold tracking-wider text-[#8a9aa8]">RCON & TELEMETRY</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <FieldShell label="Enable logStats" help="Needed for FPS metrics and richer diagnostics.">
                  <div className="flex items-center justify-between rounded border border-zinc-800 bg-[#050a0e]/40 px-3 py-2">
                    <span className="text-sm text-[#8a9aa8]">Collect engine performance stats in the log</span>
                ...
                <FieldShell label="Max FPS" help="Bohemia recommends limiting max FPS for public hosts.">
                  <Input type="number" min={30} max={240} value={runtime.max_fps} onChange={(e) => { setRuntime((prev) => ({ ...prev, max_fps: parseInt(e.target.value, 10) || 120 })); markDirty(); }} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                </FieldShell>
                <FieldShell label="Extra Startup Parameters" help="One parameter per line for advanced launch flags.">
                  <Textarea value={textEditors.startupParameters} onChange={(e) => { setTextEditors((prev) => ({ ...prev, startupParameters: e.target.value })); markDirty(); }} className="min-h-[120px] border-zinc-800 bg-[#050a0e]/60 font-mono text-xs text-white" placeholder="-profileVerbose" />
                </FieldShell>
              </CardContent>
            </Card>
            ...
            <Card className="border-zinc-800 bg-[#050a0e]/60">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-semibold tracking-wider text-[#8a9aa8]">CORE SERVER</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <FieldShell label="Server Name">
                  <Input value={game.name || ''} onChange={(e) => updateField('game.name', e.target.value)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                </FieldShell>
                <FieldShell label="Join Password">
                  <Input type="password" value={game.password || ''} onChange={(e) => updateField('game.password', e.target.value)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                </FieldShell>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldShell label="Max Players">
                    <Input type="number" value={game.maxPlayers ?? 32} onChange={(e) => updateNumber('game.maxPlayers', e.target.value, 32)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                  <FieldShell label="Mods Required By Default">
                    <div className="flex items-center justify-between rounded border border-zinc-800 bg-[#050a0e]/40 px-3 py-2">
                      <span className="text-sm text-[#8a9aa8]">Require client mods by default</span>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldShell label="Visible">
                    <div className="flex items-center justify-between rounded border border-zinc-800 bg-[#050a0e]/40 px-3 py-2">
                      <span className="text-sm text-[#8a9aa8]">Show in server browser</span>
                      <Switch checked={game.visible !== false} onCheckedChange={(value) => updateField('game.visible', value)} />
                    </div>
                  </FieldShell>
                  <FieldShell label="Cross Platform">
                    <div className="flex items-center justify-between rounded border border-zinc-800 bg-[#050a0e]/40 px-3 py-2">
                      <span className="text-sm text-[#8a9aa8]">Accept all supported platforms</span>
                    ...
                    {PLATFORM_OPTIONS.map((platform) => {
                      ...
                      return (
                        <button
                          ...
                          className={`rounded border px-3 py-2 text-xs ${selected ? 'border-tropic-gold/30 bg-tropic-gold/10 text-tropic-gold' : 'border-zinc-800 bg-[#050a0e]/40 text-[#8a9aa8]'}`}
                        >
                          {platform}
                        </button>
                      );
                ...
                <FieldShell label="Listed Admins" help="One IdentityId or SteamId per line.">
                  <Textarea value={textEditors.admins} onChange={(e) => updateListEditor('admins', 'game.admins', e.target.value)} className="min-h-[110px] border-zinc-800 bg-[#050a0e]/60 font-mono text-xs text-white" />
                </FieldShell>
              </CardContent>
            </Card>
            ...
            <Card className="border-zinc-800 bg-[#050a0e]/60">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-semibold tracking-wider text-[#8a9aa8]">OPTIONAL GAMEPLAY</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldShell label="Max View Distance">
                    <Input type="number" value={gameProps.serverMaxViewDistance ?? 2500} onChange={(e) => updateNumber('game.gameProperties.serverMaxViewDistance', e.target.value, 2500)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                  <FieldShell label="Min Grass Distance">
                    <Input type="number" value={gameProps.serverMinGrassDistance ?? 50} onChange={(e) => updateNumber('game.gameProperties.serverMinGrassDistance', e.target.value, 50)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                  <FieldShell label="Network View Distance">
                    <Input type="number" value={gameProps.networkViewDistance ?? 1000} onChange={(e) => updateNumber('game.gameProperties.networkViewDistance', e.target.value, 1000)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                  {[
                    ...
                    <FieldShell key={path} label={label}>
                      <div className="flex items-center justify-between rounded border border-zinc-800 bg-[#050a0e]/40 px-3 py-2">
                        <span className="text-sm text-[#8a9aa8]">{label}</span>
                ...
                <FieldShell label="Mission Header Override" help="JSON object for scenario metadata, time, weather, and conflict-specific tuning.">
                  <Textarea value={textEditors.missionHeader} onChange={(e) => updateJsonEditor('missionHeader', 'game.gameProperties.missionHeader', e.target.value, {})} className="min-h-[180px] border-zinc-800 bg-[#050a0e]/60 font-mono text-xs text-white" />
            ...
            <Card className="border-zinc-800 bg-[#050a0e]/60">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-semibold tracking-wider text-[#8a9aa8]">RCON</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <FieldShell label="RCON Password">
                  <Input type="password" value={rcon.password || ''} onChange={(e) => updateField('rcon.password', e.target.value)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                </FieldShell>
                <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="flex gap-2">
                      {['admin', 'monitor'].map((value) => (
                        <button
                          ...
                          className={`rounded border px-3 py-2 text-xs ${(rcon.permission || 'admin') === value ? 'border-tropic-gold/30 bg-tropic-gold/10 text-tropic-gold' : 'border-zinc-800 bg-[#050a0e]/40 text-[#8a9aa8]'}`}
                        >
                          {value}
                        </button>
                      ))}
                  ...
                  <FieldShell label="Max Clients">
                    <Input type="number" value={rcon.maxClients ?? 16} onChange={(e) => updateNumber('rcon.maxClients', e.target.value, 16)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                </div>
                <FieldShell label="Blacklist" help="One blocked command per line.">
                  <Textarea value={textEditors.rconBlacklist} onChange={(e) => updateListEditor('rconBlacklist', 'rcon.blacklist', e.target.value)} className="min-h-[100px] border-zinc-800 bg-[#050a0e]/60 font-mono text-xs text-white" />
                </FieldShell>
                <FieldShell label="Whitelist" help="If populated, only these commands may run.">
                  <Textarea value={textEditors.rconWhitelist} onChange={(e) => updateListEditor('rconWhitelist', 'rcon.whitelist', e.target.value)} className="min-h-[100px] border-zinc-800 bg-[#050a0e]/60 font-mono text-xs text-white" />
                </FieldShell>
              </CardContent>
            </Card>
            ...
            <Card className="border-zinc-800 bg-[#050a0e]/60">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-semibold tracking-wider text-[#8a9aa8]">OPTIONAL PERSISTENCE</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <p className="text-xs text-[#4a6070]">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldShell label="Auto Save Interval">
                    <Input type="number" value={persistence.autoSaveInterval ?? 10} onChange={(e) => updateNumber('game.persistence.autoSaveInterval', e.target.value, 10)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                  <FieldShell label="Hive ID">
                    <Input type="number" value={persistence.hiveId ?? 0} onChange={(e) => updateNumber('game.persistence.hiveId', e.target.value, 0)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                </div>
                <FieldShell label="Database Overrides" help="JSON object keyed by persistence database name.">
                  <Textarea value={textEditors.persistenceDatabases} onChange={(e) => updateJsonEditor('persistenceDatabases', 'game.persistence.databases', e.target.value, {})} className="min-h-[150px] border-zinc-800 bg-[#050a0e]/60 font-mono text-xs text-white" />
                ...
                <FieldShell label="Storage Overrides" help="JSON object keyed by storage name.">
                  <Textarea value={textEditors.persistenceStorages} onChange={(e) => updateJsonEditor('persistenceStorages', 'game.persistence.storages', e.target.value, {})} className="min-h-[150px] border-zinc-800 bg-[#050a0e]/60 font-mono text-xs text-white" />
            ...
            <Card className="border-zinc-800 bg-[#050a0e]/60">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-semibold tracking-wider text-[#8a9aa8]">OPTIONAL OPERATING</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <p className="text-xs text-[#4a6070]">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldShell label="Player Save Time">
                    <Input type="number" value={operating.playerSaveTime ?? 120} onChange={(e) => updateNumber('operating.playerSaveTime', e.target.value, 120)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                  <FieldShell label="AI Limit">
                    <Input type="number" value={operating.aiLimit ?? -1} onChange={(e) => updateNumber('operating.aiLimit', e.target.value, -1)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                  <FieldShell label="Slot Reservation Timeout">
                    <Input type="number" value={operating.slotReservationTimeout ?? 60} onChange={(e) => updateNumber('operating.slotReservationTimeout', e.target.value, 60)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                  <FieldShell label="Join Queue Size">
                    <Input type="number" value={operating.joinQueue?.maxSize ?? 0} onChange={(e) => updateNumber('operating.joinQueue.maxSize', e.target.value, 0)} className="border-zinc-800 bg-[#050a0e]/60 text-white" />
                  </FieldShell>
                </div>
                <FieldShell label="Disable Navmesh Streaming" help="Use engine default, disable all, or list project names one per line.">
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" size="sm" variant="outline" className="border-zinc-800 text-xs text-[#8a9aa8]" onClick={() => { clearField('operating.disableNavmeshStreaming'); setTextEditors((prev) => ({ ...prev, navmeshProjects: '' })); }}>
                      Use engine default
                    </Button>
                    <Button type="button" size="sm" variant="outline" className="border-zinc-800 text-xs text-[#8a9aa8]" onClick={() => { updateField('operating.disableNavmeshStreaming', []); setTextEditors((prev) => ({ ...prev, navmeshProjects: '' })); }}>
                  ...
                  <Textarea
                    ...
                    className="min-h-[110px] border-zinc-800 bg-[#050a0e]/60 font-mono text-xs text-white"
            ...
            <Card className="border-zinc-800 bg-[#050a0e]/60 xl:col-span-2">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-semibold tracking-wider text-[#8a9aa8]">BACKEND-MANAGED NETWORK BINDINGS</CardTitle>
              </CardHeader>
        ...
        <TabsContent value="reference" className="space-y-4">
          <div className="relative">
            ...
            <Input value={referenceQuery} onChange={(e) => setReferenceQuery(e.target.value)} placeholder="Search the Reforger config reference..." className="border-zinc-800 bg-[#050a0e]/60 pl-10 text-white placeholder:text-[#4a6070]" />
          </div>
          <div className="space-y-5">
            {Object.keys(groupedReference).length === 0 ? (
              <Card className="border-zinc-800 bg-[#050a0e]/60">
                <CardContent className="py-10 text-center text-sm text-[#4a6070]">No config reference entries match that search.</CardContent>
              </Card>
            ) : Object.entries(groupedReference).map(([section, items]) => (
              <div key={section} className="space-y-3">
                <div className="text-xs font-semibold uppercase tracking-[0.2em] text-[#4a6070]">{section}</div>
      ...
      <Dialog open={diffOpen} onOpenChange={setDiffOpen}>
        <DialogContent className="max-w-3xl border-zinc-800 bg-zinc-950 text-white">
          ...
          <div className="max-h-[70vh] overflow-auto">
            <pre className="whitespace-pre-wrap rounded bg-[#050a0e]/80 p-4 font-mono text-xs text-green-400">
              {JSON.stringify({
                log_stats_enabled: runtime.log_stats_enabled,
                max_fps: runtime.max_fps,
                startup_parameters: parseListInput(textEditors.startupParameters),
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="outline" size="sm" onClick={() => setDiffOpen(false)} className="border-zinc-700 text-[#8a9aa8]">Close</Button>
        ...
        <DialogContent className="max-w-xl border-zinc-800 bg-zinc-950 text-white">
          ...
          <div className="max-h-[60vh] space-y-2 overflow-auto">
            {configHistory.length === 0 ? (
              <p className="text-xs text-[#4a6070]">No configuration changes recorded.</p>
            ) : configHistory.map((entry, index) => (
              <div key={index} className="rounded border border-zinc-800 bg-[#050a0e]/60 p-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[#8a9aa8]">Version {configHistory.length - index}</span>
                  <span className="text-[#4a6070]">{entry.changed_at ? new Date(entry.changed_at).toLocaleString() : ''}</span>
                </div>
                {entry.changed_by ? <div className="mt-1 text-[10px] text-[#4a6070]">by {entry.changed_by}</div> : null}
                <Button size="sm" variant="outline" className="mt-2 h-6 border-zinc-800 text-[10px] text-[#8a9aa8]" onClick={() => handleRestoreHistory(entry)}>
