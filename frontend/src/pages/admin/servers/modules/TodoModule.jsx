        ...
        <CardContent className="p-4">
          <div className="flex gap-2">
            <Input value={newTodo} onChange={(e) => setNewTodo(e.target.value)}
              ...
              className="h-8 flex-1 border-zinc-800 bg-[#050a0e]/60 text-xs text-white placeholder:text-[#4a6070]" />
            <Input value={newAssignee} onChange={(e) => setNewAssignee(e.target.value)}
              placeholder="Assignee"
              className="h-8 w-32 border-zinc-800 bg-[#050a0e]/60 text-xs text-white placeholder:text-[#4a6070]" />
      ...
      {todos.length === 0 ? (
        <Card className="border-zinc-800 bg-[#050a0e]/60">
          <CardContent className="flex flex-col items-center justify-center py-12">
            <CheckSquare className="mb-2 h-8 w-8 text-[#4a6070]" />
            <p className="text-sm text-[#4a6070]">No tasks yet</p>
            <p className="mt-1 text-xs text-[#4a6070]">Add operator checklist items for this server</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-1.5">
          ...
          {[...todos.filter(t => !t.done), ...todos.filter(t => t.done)].map((todo) => (
            <Card key={todo.id} className={`border-zinc-800 bg-[#050a0e]/60 ${todo.done ? 'opacity-50' : ''}`}>
              <CardContent className="flex items-center gap-3 p-3">
                <button onClick={() => toggleTodo(todo.id)} className="shrink-0">
                  {todo.done ? (
                    <CheckCircle className="h-5 w-5 text-green-400" />
                  ) : (
                    <Square className="h-5 w-5 text-[#4a6070] hover:text-tropic-gold" />
                  )}
                </button>
                <div className="flex-1">
                  <span className={`text-xs ${todo.done ? 'text-[#4a6070] line-through' : 'text-[#d0d8e0]'}`}>
                    {todo.text}
                  </span>
                  <div className="flex items-center gap-2 mt-0.5 text-[10px] text-[#4a6070]">
                    {todo.assignee && (
                ...
                <button onClick={() => removeTodo(todo.id)} className="text-[#4a6070] hover:text-red-400">
