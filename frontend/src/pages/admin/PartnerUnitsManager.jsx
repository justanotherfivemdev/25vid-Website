  ...
  return (
    <>
      <div className="space-y-6">
        ...
        {/* Create/Edit Form */}
        {showForm && (
          <Card className="bg-[#0c1117]/80 border-tropic-gold/30">
            <CardHeader>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm text-[#8a9aa8] mb-1 block">Unit Name *</label>
                    <Input required className="bg-[#050a0e]/50 border-white/20" value={formData.name}
                      onChange={e => setFormData({ ...formData, name: e.target.value })} />
                  </div>
                  <div>
                    <label className="text-sm text-[#8a9aa8] mb-1 block">Abbreviation</label>
                    <Input className="bg-[#050a0e]/50 border-white/20" value={formData.abbreviation}
                ...
                <div>
                  ...
                  <Textarea className="bg-[#050a0e]/50 border-white/20" value={formData.description}
                    onChange={e => setFormData({ ...formData, description: e.target.value })} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm text-[#8a9aa8] mb-1 block">Contact Email</label>
                    <Input type="email" className="bg-[#050a0e]/50 border-white/20" value={formData.contact_email}
                      onChange={e => setFormData({ ...formData, contact_email: e.target.value })} />
                  </div>
                  <div>
                    <label className="text-sm text-[#8a9aa8] mb-1 block">Max Members</label>
                    <Input type="number" min={1} className="bg-[#050a0e]/50 border-white/20" value={formData.max_members}
        ...
        {/* Units List */}
        {loading ? (
          <div className="text-center text-[#4a6070] py-12">Loading partner units...</div>
        ) : units.length === 0 ? (
          ...
          <div className="space-y-3">
            {units.map(unit => (
              <div key={unit.id}>
                <Card className="bg-[#0c1117]/80 border-[rgba(201,162,39,0.12)] hover:border-tropic-gold/30 transition-colors">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 cursor-pointer" onClick={() => expandUnit(unit.id)}>
                        {expandedUnit === unit.id ? <ChevronDown className="w-4 h-4 text-[#4a6070]" /> : <ChevronRight className="w-4 h-4 text-[#4a6070]" />}
                        <div>
                          <div className="font-bold text-sm">
                            {unit.name}
                            {unit.abbreviation && <span className="text-[#4a6070] ml-2">({unit.abbreviation})</span>}
                          </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button size="sm" variant="outline" className="border-[rgba(201,162,39,0.15)] text-xs" onClick={() => startEdit(unit)}>
                          Edit
                        </Button>
                ...
                {expandedUnit === unit.id && unitDetail && (
                  <Card className="bg-[#111a24]/50 border-[rgba(201,162,39,0.15)] mt-1 ml-6">
                    <CardContent className="p-4 space-y-4">
                      ...
                      <div>
                        ...
                        {(!unitDetail.members || unitDetail.members.length === 0) ? (
                          <p className="text-xs text-[#4a6070]">No members yet. Generate an invite code below.</p>
                        ) : (
                          <div className="space-y-1">
                            {unitDetail.members.map(m => (
                              <div key={m.id} className="flex items-center justify-between bg-[#0c1117]/50 rounded px-3 py-2">
                                <div className="flex items-center gap-2">
                                  <span className="text-sm">{m.username}</span>
                                  {m.rank && <span className="text-xs text-[#4a6070]">{m.rank}</span>}
                                </div>
                                <Badge className={`text-[10px] ${m.partner_role === 'partner_admin' ? 'bg-tropic-gold/20 text-tropic-gold' : 'bg-[#111a24]'}`}>
                      ...
                      <div>
                        ...
                        {unitInvites.length === 0 ? (
                          <p className="text-xs text-[#4a6070]">No invite codes generated.</p>
                        ) : (
                          <div className="space-y-1">
                            {unitInvites.map(inv => (
                              <div key={inv.id || inv.code} className="flex items-center justify-between bg-[#0c1117]/50 rounded px-3 py-2">
                                <code className="text-tropic-gold font-mono text-xs">{inv.code}</code>
                                <div className="flex items-center gap-2">
                                  <span className="text-[10px] text-[#4a6070]">Used: {inv.use_count || 0}/{inv.max_uses || 1}</span>
                                  <Button size="sm" variant="outline" className="border-[rgba(201,162,39,0.15)] text-xs h-6 px-2" onClick={() => copyCode(inv.code)}>
