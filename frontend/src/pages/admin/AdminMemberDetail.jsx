    ...
    <AdminLayout>
      <div className="space-y-6 max-w-4xl">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={handleSaveProfile} disabled={saving} className="bg-red-700 hover:bg-red-800" data-testid="admin-save-profile-btn"><Save className="w-4 h-4 mr-2" />{saving ? 'Saving...' : 'Save Profile'}</Button>
        </div>

        {message.text && <Alert className={message.type === 'success' ? 'bg-green-900/20 border-green-700' : 'bg-red-900/20 border-red-700'}>{message.type === 'success' ? <CheckCircle className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}<AlertDescription>{message.text}</AlertDescription></Alert>}
        ...
        <Card className="bg-gray-900 border-gray-800" data-testid="admin-mission-history">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg tracking-wider flex items-center gap-2"><Target className="w-5 h-5 text-red-500" /> MISSION HISTORY</CardTitle>
              <Dialog open={missionDialogOpen} onOpenChange={setMissionDialogOpen}>
                <DialogTrigger asChild><Button size="sm" className="bg-red-700 hover:bg-red-800" data-testid="add-mission-btn"><Plus className="w-4 h-4 mr-1" />Add</Button></DialogTrigger>
                <DialogContent className="bg-gray-900 text-white border-gray-800">
                  ...
                  <form onSubmit={addMission} className="space-y-3">
                    ...
                    <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setMissionDialogOpen(false)} className="border-gray-700">Cancel</Button><Button type="submit" className="bg-red-700 hover:bg-red-800" data-testid="mission-submit-btn">Add</Button></div>
                  </form>
                </DialogContent>
              </Dialog>
            </div>
              ...
              <div className="space-y-2">
                {member.mission_history.map((m, i) => (
                  <div key={m.id || i} className="flex items-center justify-between bg-black/30 rounded p-3 border border-gray-800/50">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => removeMission(m.id)} className="text-red-500 hover:bg-red-700/10 shrink-0"><Trash2 className="w-4 h-4" /></Button>
                  </div>
                ))}
              </div>
            )}
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Dialog open={trainingDialogOpen} onOpenChange={setTrainingDialogOpen}>
                <DialogTrigger asChild><Button size="sm" className="bg-red-700 hover:bg-red-800" data-testid="add-training-btn"><Plus className="w-4 h-4 mr-1" />Add</Button></DialogTrigger>
                <DialogContent className="bg-gray-900 text-white border-gray-800">
                  ...
                  <form onSubmit={addTraining} className="space-y-3">
                    ...
                    <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setTrainingDialogOpen(false)} className="border-gray-700">Cancel</Button><Button type="submit" className="bg-red-700 hover:bg-red-800" data-testid="training-submit-btn">Add</Button></div>
                  </form>
                </DialogContent>
              </Dialog>
            </div>
              ...
              <div className="space-y-2">
                {member.training_history.map((t, i) => (
                  <div key={t.id || i} className="flex items-center justify-between bg-black/30 rounded p-3 border border-gray-800/50">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => removeTraining(t.id)} className="text-red-500 hover:bg-red-700/10 shrink-0"><Trash2 className="w-4 h-4" /></Button>
                  </div>
                ))}
              </div>
            )}
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Dialog open={awardDialogOpen} onOpenChange={setAwardDialogOpen}>
                <DialogTrigger asChild><Button size="sm" className="bg-red-700 hover:bg-red-800" data-testid="add-award-btn"><Plus className="w-4 h-4 mr-1" />Add</Button></DialogTrigger>
                <DialogContent className="bg-gray-900 text-white border-gray-800">
                  ...
                  <form onSubmit={addAward} className="space-y-3">
                    ...
                    <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setAwardDialogOpen(false)} className="border-gray-700">Cancel</Button><Button type="submit" className="bg-red-700 hover:bg-red-800" data-testid="award-submit-btn">Add</Button></div>
                  </form>
                </DialogContent>
              </Dialog>
            </div>
              ...
              <div className="space-y-2">
                {member.awards.map((a, i) => (
                  <div key={a.id || i} className="flex items-center justify-between bg-black/30 rounded p-3 border border-gray-800/50">
                    ...
                    <div className="flex items-center gap-2">{a.date && <span className="text-xs text-gray-600">{a.date}</span>}<Button size="sm" variant="ghost" onClick={() => removeAward(a.id)} className="text-red-500 hover:bg-red-700/10"><Trash2 className="w-4 h-4" /></Button></div>
                  </div>
                ))}
              </div>
            )}
