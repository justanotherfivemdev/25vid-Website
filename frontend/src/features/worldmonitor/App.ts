import type { NewsItem, Monitor, PanelConfig, MapLayers, RelatedAsset } from '@wm/types';
import {
  FEEDS,
  INTEL_SOURCES,
  SECTORS,
...
import { buildMapUrl, debounce, loadFromStorage, parseMapUrlState, saveToStorage, ExportPanel, getCircuitBreakerCooldownInfo, isMobileDevice } from '@wm/utils';
import type { ParsedMapUrlState } from '@wm/utils';
import {
  MapComponent,
  NewsPanel,
  MarketPanel,
...
import { BlackSwanPanel } from '@wm/components/BlackSwanPanel';
import { analyzeBlackSwans } from '@wm/services/black-swan-analyzer';
import type { SearchResult } from '@wm/components/SearchModal';
import { INTEL_HOTSPOTS, CONFLICT_ZONES, MILITARY_BASES, UNDERSEA_CABLES, NUCLEAR_FACILITIES } from '@wm/config/geo';
import { PIPELINES } from '@wm/config/pipelines';
import { AI_DATA_CENTERS } from '@wm/config/ai-datacenters';
import { GAMMA_IRRADIATORS } from '@wm/config/irradiators';
import type { PredictionMarket, MarketData, ClusteredEvent } from '@wm/types';
...
export class App {
  ...
  private scheduleRefresh(
    ...
    const run = async () => {
      ...
      try {
        ...
        this.inFlight.delete(name);
        if (!this._destroyed) setTimeout(run, intervalMs);
      }
    };
    setTimeout(run, intervalMs);
  }
  ...
  /**
   * Clean up the World Monitor instance — stop refresh timers, disconnect
   * WebSockets, and clear the DOM container.
   */
  public destroy(): void {
    this._destroyed = true;
    this.inFlight.clear();

    // Disconnect AIS WebSocket if active
    try { disconnectAisStream(); } catch { /* ignore */ }
    ...
    this.panels = {};
    this.newsPanels = {};
    this.signalModal = null;
    this.playbackControl = null;
    this.statusPanel = null;
    this.exportPanel = null;
    this.searchModal = null;
    this.blackSwanPanel = null;
    this.mobileWarningModal = null;
    this.pizzintIndicator = null;
  }
}
