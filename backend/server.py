...
async def startup_event():
    ...
    try:
        ...
        # Community events (community intelligence platform)
        ...
        await db.managed_servers.create_index("container_name", sparse=True)
        await db.managed_servers.create_index("data_root", sparse=True)
        await db.managed_servers.create_index("ports.game", sparse=True)
        await db.managed_servers.create_index("ports.query", sparse=True)
        await db.managed_servers.create_index("ports.rcon", sparse=True)
        await db.server_incidents.create_index("server_id")
        await db.server_incidents.create_index("status")
        await db.mod_issues.create_index("error_signature", unique=True)
        await db.mod_issues.create_index("status")
        await db.mod_issues.create_index("designated_area")
        await db.mod_issues.create_index([("troublesome", 1), ("mod_id", 1)])
        await db.server_backups.create_index("server_id")
        await db.server_schedules.create_index("server_id")
        await db.server_schedules.create_index("id", unique=True)
        await db.server_schedules.create_index("next_run")
        await db.server_webhooks.create_index("id", unique=True)
        await db.server_notifications.create_index("id", unique=True)
        await db.server_notifications.create_index([("server_id", 1), ("status", 1)])
        await db.server_notifications.create_index([("server_id", 1), ("dedupe_key", 1)], unique=True)
        ...
        await db.log_error_types.create_index("review_status")
        await db.log_error_types.create_index("attribution_type")
        await db.log_error_types.create_index("designated_area")
        await db.log_error_types.create_index("actionability")
        await db.log_error_types.create_index("linked_mod_guid")
        await db.log_error_types.create_index([("troublesome", 1), ("linked_mod_guid", 1)])
        ...
        await db.log_occurrences.create_index("source_stream")
